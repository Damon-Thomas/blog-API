...
const logIN = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!userExists)
      return res
        .status(400)
        .json({ message: "user does not exist", failure: true });

    // check if password is correct
    if (!(await bcrypt.compare(req.body.password, userExists.password)))
      return res
        .status(400)
        .json({ message: "incorrect password", failure: true });
    else {
      req.user = userExists;
    ...
    return res.status(400).json({ message: error.message, failure: true });
  }
});
