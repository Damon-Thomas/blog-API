...
const getPostandComments = asyncHandler(async (req, res) => {
  console.log("getPostandComments", req.params.postId);
  ...
  const author = await postQueries.getAuthor(post.authorId);
  console.log(post, comments, author);
  res.json({ post, comments, author });
});
...
const updatePost = asyncHandler(async (req, res) => {
  ...
  if (req.user.id !== post.authorId) {
    ...
    const updatedPost = await postQueries.updateBlogPost(
      req.params.postId,
      req.body
    );
